...
5. **Personal Egress 3LO OAuth Engine (Connected Accounts)**: For third-party services (Google Home, Google Workspace, Slack, GitHub, Notion), MCG acts as an OAuth2 client and credential vault. Users connect accounts through the UI via `GET /api/oauth/egress/authorize/{serverId}` and callback handler. Tokens are stored in `IUserSecretStore` (Database or Vault) and automatically refreshed in the background during upstream tool calls via `OAuthEgressTokenManager`. For details, see the [**Per-User OAuth Delegation & Connected Accounts Flow**](docs/auth-flows/per-user-oauth-flow.md).
