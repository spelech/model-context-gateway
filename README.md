...
![Version](https://img.shields.io/badge/version-v5.17.3-orange?style=for-the-badge)
...
![Tests](https://img.shields.io/badge/tests-1%2C168%20passing-2ea44f?style=for-the-badge)
...
| :--- | :--- | :--- |
| **`v5.17.3`** | 2026-09-26 | fix(testbench,transports): Test Bench Direct Dispatch, Alias Resolution, Zero-Warning SSE Disconnection, and Resilient Capability Probing. Eliminated false-alarm disconnect warnings during backend connection disposal by adding cancellation and disposal guards in `SseTransport` reader and ping loops; resolved `KeyNotFoundException` crashes on non-local prompt execution (`POST /api/test/prompts/get`) and resource reads (`POST /api/test/resources/read`) by dispatching directly to connected backends and un-virtualizing `mcp://` URIs; expanded `GetServerByIdAsync` to resolve servers by canonical ID or Alias across SQL providers; and demoted offline discovery probes to warnings preventing system log flooding (`MCP-35`). |
| **`v5.17.2`** | 2026-09-24 | feat(testbench,ui): Test Bench Server Grouping, Multi-Delimiter Routing Parity, Interactive Tool Hints, and Full Viewport Layout Inspector Matrix. Implemented multi-delimiter (`/`, `__`, `:`) server resolution fallback and prefix stripping across tool and prompt execution endpoints (`POST /api/test/call`, `POST /api/test/prompts/get`) (`MCP-35`); added centralized `parseNamespacedName` utility; redesigned Test Bench UI with dynamic server grouping, clean bare tool selectors, interactive tool and prompt hint callout banners, enum dropdown form controls with type coercion, and atomic bulk pre-fill argument synchronization (`UI-132`); implemented responsive mobile card reflow across all data tables and expanded Playwright layout inspector to 34 tests comprehensively auditing every dashboard view, sub-navigation tab, and modal across Desktop 1080p, Mobile Samsung Galaxy S25+, and Tablet Samsung Galaxy Tab S10 Lite with zero horizontal overflow (`UI-07`). |
| **`v5.17.1`** | 2026-09-23 | fix(transports,ui): Upstream Notification Resilience, Modal Scrolling & Viewport Boundary Protection, and Comprehensive Layout Inspector Audit. Handled HTTP 404 and 405 gracefully in `HttpTransport` notification dispatch preventing broadcast exceptions on unsupported upstream backends (`TRANS-03`); added viewport bounds, max-height constraints, and internal scrolling across all modal dialogs preventing clipping on smaller viewports (`UI-07`); and expanded Playwright layout inspector suite across tabs, drawer sidebars, and modals verifying zero horizontal overflow, element clipping, or layout shifts. |
