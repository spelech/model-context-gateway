...
For upstream MCP servers that require per-user authentication (such as Google Home, Google Workspace, Slack, GitHub, Notion, or Jira), Model Context Gateway provides a **Personal Egress 3LO OAuth Engine ("Connected Accounts")**:
...
- **Detailed Protocol Specification**: For sequence diagrams, data boundary diagrams, and configuration walkthroughs, read the [**Per-User OAuth Delegation & Connected Accounts Flow**](auth-flows/per-user-oauth-flow.md).
